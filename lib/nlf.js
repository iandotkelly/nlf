...
function createModule(moduleData, licenseFileNamePartsToFind, callback) {
	...
	co(function* () {
		for (var i = 0; i < licenseFileNamePartsToFind.length; i += 1) {
			var fileNamePart = licenseFileNamePartsToFind[i];
			var separateSourceCollectionName = fileNamePart.length === 2;
			var licenseFiles = yield findPotentialLicenseFilesThunk(
				directory, 
				'*' + (separateSourceCollectionName ? fileNamePart[0] : fileNamePart) + '*'
			);
			yield addFilesThunk(
				licenseFiles,
				module.getLicenseCollection(
					separateSourceCollectionName ? fileNamePart[1] : fileNamePart
				)
			);
		}
...
function processOptionLicenseFileNamePartsToFind(options) {
	if (!options.licenseFileNamePartsToFind) {
		options.licenseFileNamePartsToFind = [
			['li@(c|s)en@(c|s)e', 'license'],
			'readme'
		];
	} else if (Object.prototype.toString.call(options.licenseFileNamePartsToFind) 
		!== '[object Array]') {
...
function processOptions(options) {
	...
	processOptionLicenseFileNamePartsToFind(options);
