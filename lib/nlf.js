...
function createModule(moduleData, callback) {
	...
	co(function* () {
		try {
			...
			yield addFilesThunk(licenseFiles, module.licenseSources.license);

			// glob for readme files
			var readmeFiles = yield findPotentialLicenseFilesThunk(
				directory, '*readme*'
			);
			yield addFilesThunk(readmeFiles, module.licenseSources.readme);
